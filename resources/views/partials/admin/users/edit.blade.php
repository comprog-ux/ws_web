    ...
    <div class="col-sm-8">
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                ...
                <div ng-if="user.user | isEmpty">
                    {{_('Användaren verkar inte finnas tillgänglig eller så har du inte administratörsbehörighet.')}}
                </div>
               <form role="form" name="settingsForm" novalidate="">
                    <div class="row">
                        <div class="col-sm-5 padding-top-5">{{_('Aktiv')}}</div>
                        <div class="col-sm-7">
                            <div class="btn-group">
                                <label class="btn btn-success" ng-model="user.user.active" uib-btn-radio="1" uncheckable>{{_('Ja')}}</label>
                                <label class="btn btn-success" ng-model="user.user.active" uib-btn-radio="0" uncheckable>{{_('Nej')}}</label>
                            </div>
                        </div>
                    </div>
                    <hr>
                    ...
                    <hr>
                    <div class="row">
                        <div class="col-sm-5 padding-top-5">{{_('Aktiveringskod')}}</div>
                        <div class="col-sm-7">
                            <input type="text" class="form-control" ng-model="user.user.activation_code" ng-enter="user.saveUserprofile()" placeholder="{{_('Aktiveringskod')}}">
                        </div>
                    </div>
                    <hr>
                    <div class="row">
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-7">
                            <div ng-if="user.user.clubs.length > 0" style="margin-bottom: 10px;">
                                Vald klubb: <% user.user.clubs[0].name %> <a ng-click="user.removeClubAssociation(user.user)" href="javascript:void(0)">(Ta bort koppling)</a>
                            </div>
                            ...
                            <input type="text"
                                   ...
                                   uib-typeahead="searchclub for searchclubs in user.searchForClubs($viewValue);"
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            <button class="btn btn-default" ui-sref="admin.users.show({user_id: user.user.user_id})">{{_('Avbryt')}}</button>
                        </div>
                        <div class="col-sm-6 text-right">
                            <button class="btn btn-primary" ng-click="user.saveUser(user.user)">{{_('Spara')}}</button>
                        </div>
                    </div>
                </form>
            </div>
    ...
    <div class="col-sm-4">
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                ...
                <p>{{_('Om användaren inte har haft någon registrerad e-post sedan tidigare skickas ett aktiveringsmail till e-postadressen som anges. Mailet innehåller en aktiveringslänk som gör det möjlighet för användaren att aktivera sitt konto.')}}</p>
