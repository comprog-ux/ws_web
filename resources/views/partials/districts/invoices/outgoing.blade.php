<div class="panel panel-default">
    ...
    <div class="panel-body">
        <div class="row">
            ...
            <div class="col-sm-4">
                <select class="form-control" ng-model="invoices.invoices.orderby" ng-change="invoices.updatePage();">
                    <option value="">{{_('Fakturadatum')}}</option>
                    <option value="expiration_date" ng-selected="invoices.invoices.orderby == 'expiration_date'">{{_('Förfallodatum')}}</option>
                    <option value="paid_at" ng-selected="invoices.invoices.orderby == 'paid_at'">{{_('Betalningsdatum')}}</option>
                    <option value="recipient_name" ng-selected="invoices.invoices.orderby == 'recipient_name'">{{_('Mottagare')}}</option>
                    <option value="amount" ng-selected="invoices.invoices.orderby == 'amount'">{{_('Belopp')}}</option>
                    <option value="invoice_nr" ng-selected="invoices.invoices.orderby == 'invoice_nr'">{{_('Fakturanr')}}</option>
                </select>
            </div>
            <div class="col-sm-4">
        ...
        <table class="table table-hover table-bordered">
            ...
            <tbody>
                <tr ng-repeat="invoice in invoices.invoices.data">
                    <td nowrap ui-sref="districts.show.invoices.show({districts_id: district.district.id, id:invoice.id})"><% invoice.invoice_date %></td>
                    <td ui-sref="districts.show.invoices.show({districts_id: district.district.id, id:invoice.id})"><% invoice.invoice_reference %></td>
                    <td ui-sref="districts.show.invoices.show({districts_id: district.district.id, id:invoice.id})"><% invoice.recipient_name %></td>
                    ...
                    <td class="text-right nowrap">
                        <button ng-click="invoices.download(invoice);" class="btn btn-xs btn-default"><i class="fa fa-download"></i></button>
                    </td>
                    <td class="text-right nowrap">
                        <button class="btn btn-xs btn-default" ui-sref="districts.show.invoices.show({districts_id: district.district.id, id:invoice.id})">{{_('Visa')}}</button>
                    </td>
                </tr>
            </tbody>
        </table>
...
<script type="text/ng-template" id="DistrictInvoicePaymentModal.html">
